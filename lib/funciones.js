...
function cargarDetallesTarea () {
	...
	var html = '<div data-role="content"><div data-role="controlgroup" data-type="horizontal" align="center"><a onclick="cargarTrkTarea(' + tarea + ')" data-role="button" data-theme="b">Ver Tracking</a><a onclick="ingresarTrk(' + tarea + ')" data-role="button" data-theme="b">Ingresar Tracking</a></div><div data-role="controlgroup" data-type="horizontal" align="center"><a onclick="reasignarTarea(' + tarea + ')" data-role="button" data-theme="b">Reasignar</a><a onclick="consultarRRHHtarea(' + tarea + ')" data-role="button" data-theme="b">Consultar RRHH</a></div>';
...
function cargarDetallesProyecto() {
	...
	var html = '<div data-role="content"><div data-role="controlgroup" data-type="horizontal" align="center"><a onclick= "redireccionar(\'ConsultaProdAbiertos.html\')" data-role="button" data-theme="b">Productos</a></div>';
	//<a onclick = "redireccionar(\'DetalleRRHHProyecto.html\')" data-role="button" data-theme="b">Consultar RRHH</a>
...
function cargarDatosRRHH_t() {
	...
	var html = '<div data-role="content"><p>';
		...
		jQuery.get("http://localhost:8000/selectTareaRol/"+tarea,function(resultado2){
		data = jQuery.parseJSON(resultado);
		data2 = jQuery.parseJSON(resultado2);
		$.each(data, function(i, elemento){
			html+='<option value="RRHH '+elemento.rrhh+'">'+elemento.rrhh+'</option>';
			});				
			html += '</select></p><p><label for="selectRol" class="select">Rol:</label><select name="selectRol" id="selectRol" data-theme="b"><option value="">Seleccionar</option>';
		$.each(data2, function(i, elemento){		
			html+='<option value="Rol '+elemento.rol+'">'+elemento.rol+'</option>';
			});
		html = html + '</select></p></div>';
 ...
 * 20120810 Alejandra
 */
function agregarRRHHtarea(tarea) {
...
function cargarProyRRHH() {
	var proy = localStorage.proy;
	var htmlh = '<h1>Consulta RRHH - Producto # '+ proy + '</h1>';
	$(htmlh).appendTo("#headertarea2");
	
	var html = '<div data-role="content"><div data-role="controlgroup" data-type="horizontal" align="center"><a onclick="agregarRRHHprod(' + proy + ')" data-role="button" data-theme="b">Agregar</a></div>';
	
	html = html + '<p><ul data-role="listview" data-inset="true">';
	
	jQuery.get("http://localhost:8000/listaRecursosT/"+proy,function(resultado){
	data = jQuery.parseJSON(resultado);
	$.each(data, function(i, elemento){
		html = html + '<li>'+elemento.rrhh+'</br><br><p>'+elemento.rol+'</p></a></li>';
		});
		html = html + '</ul></p></div>';
		$(html).appendTo("#detalle_proyRRHH");
		location.href="DetalleRRHHProy.html\#detalleProyRRHH";
	});
}
 ...
 * 20120810 MT
 */
function agregarRRHHprod(tarea) {
